...
local function _startPlayExpiration()
    _EXPIRATION_TIMER_CANCEL = timer.performWithDelay(Constants.NUM_WAIT_MILLISECONDS_TO_HIDE_AD_IF_NOT_SHOW, function()
        if adListener then
            Toast.show("Can't show ad. Please try later", {duration="short"})
...
local function _showRewarded()
    ...
    if Ads then 
        if Ads.isLoaded(true) then
            ...
            Ads.load(true)
            Toast.show("Ad not ready to show", {duration="long"})
        end
    else
        Toast.show("Ads not supported", {duration="long"})
    end
    return false
end
...
local function _isActive()
    return globals_persistence:get("ads")
end
