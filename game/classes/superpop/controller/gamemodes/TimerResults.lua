...
local btnBackRelease = function(event)
    if event == nil or "ended" == event.phase then
        ...
        Controller:goNextSceneVerifyingAdsRate({scene="classes.superpop.controller.scenes.Frontend", gameMode="Timer", view=objScene.view})
    end
    return true
end
...
function objScene:create(event)
    ...
    local isScoreHigh, isScoreHighToday = Controller:updateScore({nScore=params.numScore, gameMode="Timer"})
    local numScoreHigh, numScoreHighToday = globals_persistence:get("nHighScoreTimer"), globals_persistence:get("nHighScoreTimerToday")
    ...
    local grpRank = Ranking:new({isHorizontal=true, gameMode="Timer"})
    ...
    local btnReloadEvent = function(event)
        if "ended" == event.phase then
            Jukebox:dispatchEvent({name="playSound", id="button"})
            Controller:goNextSceneVerifyingAdsRate({scene="classes.superpop.controller.scenes.Frontend", gameMode="Timer", reloadMode="Timer", view=objScene.view})
        end
    end
    ...
    local _btnDoShareEvent = function(event)
        if "ended" == event.phase then
            ...
            if isAvailable then
                ...
                local imgFG = display.newSprite(_SHT_UTIL, { {name="s", start=72, count=1} })
                ...
                native.showPopup("social", {
                    service = serviceName, 
                    message = strScore .. " POINTS on Timer mode! " .. Constants.STR_SOCIAL_SHARE_MESSAGE,
