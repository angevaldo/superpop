...
local btnBackRelease = function(event)
    if event == nil or "ended" == event.phase then
        ...
        Controller:goNextSceneVerifyingAdsRate({scene="classes.superpop.controller.scenes.Frontend", gameMode="Bomb", view=objScene.view})
    end
    return true
end
...
function objScene:create(event)
    ...
    local isScoreHigh, isScoreHighToday = Controller:updateScore({nScore=params.numScore, gameMode="Bomb"})
    local numScoreHigh, numScoreHighToday = globals_persistence:get("nHighScoreBomb"), globals_persistence:get("nHighScoreBombToday")
    ...
    local grpRank = Ranking:new({isHorizontal=true, gameMode="Bomb"})
    ...
    local btnReloadEvent = function(event)
        if "ended" == event.phase then
            Jukebox:dispatchEvent({name="playSound", id="button"})
            Controller:goNextSceneVerifyingAdsRate({scene="classes.superpop.controller.scenes.Frontend", gameMode="Bomb", reloadMode="Bomb", view=objScene.view})
        end
    end
    ...
    local _btnDoShareEvent = function(event)
        if "ended" == event.phase then
            ...
            if isAvailable then
                ...
                local imgFG = display.newSprite(_SHT_UTIL, { {name="s", start=74, count=1} })
                ...
                native.showPopup("social", {
                    service = serviceName, 
                    message = strScore .. " POINTS on Bomb mode! " .. Constants.STR_SOCIAL_SHARE_MESSAGE,
